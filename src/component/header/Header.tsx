...
import { useNavigate, useLocation } from "react-router-dom"; // Added useLocation for route change detection
...
const Header = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0); // This will scroll to top every time the route changes
  }, [location]);
          ...
          <nav className="hidden lg:flex space-x-6 text-xl font-semibold relative">
            ...
            >
              <button className="flex items-center gap-1 hover:text-blue-700 transition-colors duration-200">
                Product
                <ChevronDownIcon className={`transition-transform duration-200 ${hoveredMenu === "product" ? "rotate-180" : ""}`} />
              </button>
              <div
                className={`absolute top-full left-0 mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden transition-all duration-200 ${
                  hoveredMenu === "product" ? "opacity-100 visible transform translate-y-0" : "opacity-0 invisible transform -translate-y-2"
                }`}
              >
                <div className="py-2">
            ...
            >
              <button className="flex items-center gap-1 hover:text-blue-700 transition-colors duration-200">
                ...
                <ChevronDownIcon className={`transition-transform duration-200 ${hoveredMenu === "corporate" ? "rotate-180" : ""}`} />
              </button>
              <div
                className={`absolute top-full left-0 mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden transition-all duration-200 ${
                  hoveredMenu === "corporate" ? "opacity-100 visible transform translate-y-0" : "opacity-0 invisible transform -translate-y-2"
                }`}
              >
                  ...
                  {corpGovLinks.map((link) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-sm text-gray-800 group-hover/item:text-blue-700">{link.name}</div>
                      </div>
                    </div>
                  ))}
                </div>
